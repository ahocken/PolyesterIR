...
main_folder_path = r'C:/Users/XXXXX/Food Noised Samples/NIR'
...
for i in stand_devs:
    ...
    for material in materials:
        ...
        for file_name in os.listdir(material_folder_path):
            if file_name.endswith('.CSV') or file_name.endswith('.csv'):
                ...
                if is_first_file:
                    column_headers = df.iloc[11:163, 0].tolist()
                    is_first_file = False
                    
                # Extract the second column
                second_column = df.iloc[11:163, 1]
    ...
    output_path = os.path.join(main_folder_path, f'CombinedSpectraDataFrame_MaxMinNorm_NIR_STD_{i}.csv')
    combined_df.to_csv(output_path, index=False)

    sample_names.insert(0,'SampleName')

    output_path_labels = os.path.join(main_folder_path, f'TrashVPolyesterLabels_NIR_STD_{i}.csv')
    ...
    output_path_labels_polyester = os.path.join(main_folder_path, f'PolyesterTypeLabels_NIR_STD_{i}.csv')

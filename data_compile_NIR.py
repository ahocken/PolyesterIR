...
## the first three letters of the file name, the code assigns label (0 - 4) 
...
## with no headers. This is how the MRL iS50 FTIR exports its data to CSV
...
folder_path = r'C:/Users/XXX/NIR Data'
...
for material in materials:
    ...
    for file_name in os.listdir(material_folder_path):
        if file_name.endswith('.CSV') or file_name.endswith('.csv'):
            ...
            if is_first_file:
                column_headers = df.iloc[12:164, 0].tolist()
                is_first_file = False
        
            # Extract the second column
            second_column = df.iloc[12:164, 1]
...
output_path = os.path.join(result_folder_path, 'CombinedSpectraDataFrame_MaxMinNorm_NIR.csv')
combined_df.to_csv(output_path, index=False)

sample_names.insert(0,'SampleName')

output_path_labels_polyester = os.path.join(result_folder_path, 'PolyesterTypeLabels_NIR.csv')
